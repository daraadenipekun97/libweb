        ...
        <div className="search-container">
            {
                searchedBooks.books.length !== 0 ? (
                    <SingleBook datas={searchedBooks.books} searchBar="" title={`Books result for ${params.id}`} />
...

            {
                searchedBooks.authors.length !== 0 ? (
                    <SingleAuthor datas={searchedBooks.authors} title={`Author result for ${params.id}`} seeAllText = {false}  />
                ) :  <></>
            }
           
            {
                searchedBooks.books.length === 0 && searchedBooks.genres.length === 0  &&  searchedBooks.authors.length === 0 ? 
                (
