...
const ReadVoteArticle = () => {
  ...
  const { articleById, voteArticleSuccess, voteArticleFailure } = useSelector(
  ...
  const callParagaraphs = () => {
    if (Object.keys(articleById).length !== 0) {
      let paragraphs = articleById?.content
        .split(/(?<=\.)\s+(?=[A-Z])/)
    ...
    <>
      ...
      <div className="article-writeup-wrapper">
        <div className="backbtn">
          <a href="javascript:history.back()" className="back-to-category">
            Back To Category
          </a>
        </div>
        <div className="article-writeup">
          {paragraph?.map((item) => (
            <p className="article-text">{item.splitText.replace(/(<([^>]+)>|&\w+;)/gi, "")}</p>
          ))}
        </div>
        ...
        <div className="profile">
          <div class="profile-img">
            <img
              src={
                articleById?.user?.image_data?.includes("avatar.png")
                  ? Avatar
                  : articleById?.user?.image_data
              }
              alt=""
            />
          </div>
        ...
        <div className="article-vote-share">
          {articleVoted ? (
            ...
            <div className="article-vote" onClick={() => handleVote()}>
              <AiOutlineHeart size={25} color="red" />
              <small>Vote this article. Article has {articleById?.votes?.length} vote(s)</small>
            </div>
          )}
          <small style={{ color: "brown" }}>
            *you can only vote for your article and one other article throughout the challenge
          </small>
        </div>
        ...
        <div className="social">
          ...
          <div>
            <FacebookShareButton className="social-btn" url={`${window.location.href}`}>
