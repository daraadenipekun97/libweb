...
import { fetchProfile, fetchSubscriptionDetails, cancelTrial, restoreCancelTrialInitial, cancelSubscription, restoreCancelSubscriptionInitial } from '../../Actions'
...
const Subscription = () => {
  ...
  const { profileData, subscriptionDetails, cancelTrialSuccess, cancelTrailFailure, cancelSubscriptionSuccess, cancelSubscriptionFailure  } = useSelector((state) => state.profile);
  ...
  return (
    <div className='subscription-container'>
      ...
      <hr />
      {
        Object.keys(subscriptionDetails).length > 0 && subscriptionDetails.subscription !== null  ? (
          <>
            ...
            {
              subscriptionDetails.subscription.title === "Trial"  && profileData.cancel_trial === 0?<h3 className='subscription-status-green'>{`You're subscribed to ${subscriptionDetails.subscription.title} Plan`}</h3> 
              : subscriptionDetails.subscription.title === "Trial"  && profileData.cancel_trial === 1? (
          ...
          {
              subscriptionDetails.subscription.title !== "Trial"  && profileData.cancel_subscription === 0 ?<h3 className='subscription-status-green'>{`You're subscribed to ${subscriptionDetails.subscription.title} Plan`}</h3> 
              : subscriptionDetails.subscription.title !== "Trial"  && profileData.cancel_subscription === 1? (
                <h3 className='subscription-status'>{`You have canceled your ${subscriptionDetails.subscription.title} Plan`}</h3>
              ) :<></>
            ...
            {
              subscriptionDetails.last_payment ? <p className='sub-note'>{`Subscription Start Date: ${moment(subscriptionDetails.last_payment).format("MMM Do YYYY")}`}</p>
              : <></>
            }
            {
              subscriptionDetails.expiry_date?<p className='sub-note'>{`Subscription End Date: ${moment(subscriptionDetails.expiry_date).format("MMM Do YYYY")}`}</p> : <></>
...

            {
              ...
              ) :profileData.cancel_trial === 1 && subscriptionDetails.subscription.title === "Trial" ? (
            ...
            {
              subscriptionDetails.subscription.title !=="Trial" && profileData.cancel_subscription === 0 ? (
      ...
      <div className="card">
          ...
          {
             Object.keys(subscriptionDetails).length > 0? (<></>) :<a href="/home/subscription" className='select-btn'>Select</a>
          }
      </div>
